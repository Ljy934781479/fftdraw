...
int CFFT::outSpecial(int n, int t, Cpx* ratioList)
{
	int k = -n;
	Cpx sum(0, 0);
	Cpx* c = ratioList;
	while (c)
	{
		pair<int, int> p = getPixPos(&sum);
		Cpx cur = (*c) * euler(k * -2 * PI * t);
		drawCpx(cur, false, GREEN, _bmp,p.first,p.second);
		sum = sum + cur;
		k++;
		c = c->next;
	}
	string str = "d:\\CodeWork\\";
	str += to_string(_progress) + ".bmp";
	return 0;
}
